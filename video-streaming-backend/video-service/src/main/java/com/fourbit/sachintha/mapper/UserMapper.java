...
public class UserMapper {
    public static User mapToUser(UserDto userDto) {
        ...
        List<User> subscriptions = Optional.ofNullable(userDto.getSubscriptions()).orElse(Collections.emptyList())
                .stream()
        ...
        List<User> subscribers = Optional.ofNullable(userDto.getSubscribers()).orElse(Collections.emptyList()).stream()
                .map(userObj -> mapToUser(userObj)).toList();
        List<Video> videoHistory = Optional.ofNullable(userDto.getVideoHistory()).orElse(Collections.emptyList())
                .stream()
    ...
    public static UserDto mapToUserDto(User user) {
        ...
        List<UserDto> subscriptions = Optional.ofNullable(user.getSubscriptions()).orElse(Collections.emptyList())
                .stream()
        ...
        List<UserDto> subscribers = Optional.ofNullable(user.getSubscribers()).orElse(Collections.emptyList()).stream()
                .map(userObj -> mapToUserDto(userObj)).toList();
        List<VideoDto> videoHistory = Optional.ofNullable(user.getVideoHistory()).orElse(Collections.emptyList())
                .stream()
